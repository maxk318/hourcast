...
function updateCurrentLocation(lat, lon, manual) {
  ...
  // Raw coords, hidden on the settings page — lets it build the "10 nearest
  // tide stations" dropdown from the same location weather already resolved,
  // without a second geolocation prompt.
  setClaySetting('CURRENT_LAT', lat4);
  setClaySetting('CURRENT_LON', lon4);
...
Pebble.addEventListener('webviewclosed', function (e) {
  ...
  delete dict[messageKeys.CURRENT_LAT];
  delete dict[messageKeys.CURRENT_LON];
...
// Full NOAA tide-prediction station list (~3,500 stations), regenerated by
// tools/generate_tide_stations.py. Used both for auto-detecting the nearest
// station here and, in custom-clay.js, for populating the manual dropdown
// with the 10 closest to the phone's location.
...
function sendTide(lat, lon) {
  ...
  xhr.onload = function () {
    try {
      ...
      // Find the next high and low tide within the 12-hour window, the same
      // way sunrise/sunset find their transition: first occurrence wins. A
      // point is a turning point if it's higher (or lower) than both its
      // 6-minute neighbors — precise enough without a second NOAA request.
      var windowStart = utcHour0.getTime(), windowEnd = windowStart + 12 * 3600000;
      var hiK = -1, hiStr = '', loK = -1, loStr = '';
      for (var p = 1; p < tides.predictions.length - 1 && (hiK < 0 || loK < 0); p++) {
        var pms = new Date(tides.predictions[p].t.replace(' ', 'T') + 'Z').getTime();
        if (pms < windowStart || pms >= windowEnd) continue;
        var v0 = parseFloat(tides.predictions[p - 1].v);
        var v1 = parseFloat(tides.predictions[p].v);
        var v2 = parseFloat(tides.predictions[p + 1].v);
        var d = new Date(pms);
        var slotK = d.getHours() % 12;
        var mm = d.getMinutes();
        var dispH = (slotK === 0) ? 12 : slotK;
        var str = dispH + ':' + (mm < 10 ? '0' : '') + mm;
        if (hiK < 0 && v1 > v0 && v1 > v2) { hiK = slotK; hiStr = str; }
        if (loK < 0 && v1 < v0 && v1 < v2) { loK = slotK; loStr = str; }
      }

      var rawTides = {};
      var minLevel = Infinity, maxLevel = -Infinity;

      // Key by clock-dial slot (local hour % 12), same convention the watch
      // uses for ICON_k/TEMP_k, not by hours-from-now — otherwise the tide
      // level lands on the wrong dial position.
      for (var h = 0; h < 12; h++) {
        var ms = utcHour0.getTime() + h * 3600000;
        if (levelByUtcMs.hasOwnProperty(ms)) {
          var v = levelByUtcMs[ms];
          var k = new Date(ms).getHours() % 12;
      ...
      if (Object.keys(rawTides).length > 0 && maxLevel > minLevel) {
        var tideDict = {};
        for (var k = 0; k < 12; k++) {
          if (rawTides.hasOwnProperty(k)) {
            tideDict['TIDE_' + k] = Math.round((rawTides[k] - minLevel) / (maxLevel - minLevel) * 100);
          }
        }
        tideDict['TIDE_VALID'] = 1;
        if (hiK >= 0) { tideDict['TIDE_HI_K'] = hiK; tideDict['TIDE_HI_STR'] = hiStr; }
        if (loK >= 0) { tideDict['TIDE_LO_K'] = loK; tideDict['TIDE_LO_STR'] = loStr; }
